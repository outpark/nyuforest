...
bot.dom.getOverflowState = function(a, b) {
  ...
  for (var f = bot.dom.getClientRegion(a, b), g = goog.dom.getOwnerDocument(a), h = g.documentElement, l = g.body, q = bot.dom.getEffectiveStyle(h, "overflow"), p, k = c(a);k;k = c(k)) {
    ...
    if ("visible" != r.x || "visible" != r.y) {
      ...
      if (m && "hidden" == r.x || s && "hidden" == r.y) {
        return bot.dom.OverflowState.HIDDEN;
      }
      if (m && "visible" != r.x || s && "visible" != r.y) {
        m = e(k);
        s = f.bottom < n.top - m.y;
        if (f.right < n.left - m.x && "visible" != r.x || s && "visible" != r.x) {
          return bot.dom.OverflowState.HIDDEN;
        }
...
bot.dom.getClientRect = function(a) {
  function b(a) {
    ...
    if (c) {
      var d = parseFloat(c[1]), e = parseFloat(c[2]), p = parseFloat(c[3]), k = parseFloat(c[4]), r = parseFloat(c[5]), c = parseFloat(c[6]), n = f.left + f.width, m = f.top + f.height, s = f.left * d, d = n * d, t = f.left * e, e = n * e, u = f.top * p, p = m * p, v = f.top * k, n = m * k, m = s + u + r, k = t + v + c, u = d + u + r, v = e + v + c, s = s + p + r, t = t + n + c, r = d + p + r, c = e + n + c;
...
Dispatcher.prototype.init_ = function() {
  ...
  this.bind_("/session/:sessionId/element/:id/rect").on(Request.Method.GET, Dispatcher.executeAs("getElementRect"));
  ...
  if (0 != c.indexOf("/hub")) {
    ...
    if (d) {
      try {
        ...
        goog.log.error(this.log_, "Error processing request", f), b.sendError(Response.INTERNAL_ERROR, JSON.stringify({status:bot.ErrorCode.UNKNOWN_ERROR, value:fxdriver.error.toJSON(f)}), "application/json");
      }
    } else {
...
fxdriver.modals.signalOpenModal = function(a, b) {
  ...
  if (c && c.response_) {
    ...
    "executeAsyncScript" != c.response_.name || c.response_.responseSent_ ? (d.value = b, d.send()) : fxdriver.modals.isModalPresent(function(a) {
      ...
      d.sendError(new WebDriverError(bot.ErrorCode.UNEXPECTED_ALERT_OPEN, f, {alert:{text:b}}));
    }, 2E3);
  }
};
...
WebElement.clickElement = function(a, b) {
  ...
  if (!q && this.enableNativeEvents && e && f && h && l) {
    ...
    try {
      ...
      if (goog.log.info(WebElement.LOG_, "Detected error when clicking", t), "NS_ERROR_NOT_IMPLEMENTED" != t.name) {
