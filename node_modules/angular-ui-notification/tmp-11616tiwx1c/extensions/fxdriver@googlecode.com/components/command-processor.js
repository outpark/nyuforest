...
bot.dom.getClientRect = function(a) {
  function b(a) {
    ...
    if (c) {
      var d = parseFloat(c[1]), e = parseFloat(c[2]), p = parseFloat(c[3]), l = parseFloat(c[4]), r = parseFloat(c[5]), c = parseFloat(c[6]), n = f.left + f.width, m = f.top + f.height, s = f.left * d, d = n * d, t = f.left * e, e = n * e, u = f.top * p, p = m * p, v = f.top * l, n = m * l, m = s + u + r, l = t + v + c, u = d + u + r, v = e + v + c, s = s + p + r, t = t + n + c, r = d + p + r, c = e + n + c;
    ...
    for (var q = c.toUpperCase(), p = 0;p < c.length;p++) {
      ...
      if ("\ue000" == l) {
        ...
        if (r) {
          ...
          Utils.keyEvent(a, b, "keypress", l, n, e, r || h, k, d, !s);
      ...
      if (g = k.fxdriver) {
        if (k.getBrowser().contentWindow) {
          if (g.modalOpen && "getAlertText" != c.name && "setAlertValue" != c.name && "acceptAlert" != c.name && "dismissAlert" != c.name) {
            ...
            goog.log.error(nsCommandProcessor.LOG_, "Sending error from command " + c.name + " with alertText: " + k);
            d.sendError(new WebDriverError(bot.ErrorCode.UNEXPECTED_ALERT_OPEN, "Modal dialog present", {alert:{text:k}}));
          } else {
